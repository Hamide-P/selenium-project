...
public class Dropdowns_T6_T7_T8 {
    ...
    WebDriver driver; //  We cannot use WebDriver separately for each test when we only declare it in 'test1'; we should declare it at the class level to use it for all the tests
    ...
    public void dropdowns_task6() throws InterruptedException {
        ...
        Thread.sleep(2000); // we use it only for presentation purpose
        ...
        //  String actualYear = yearDropdown.getFirstSelectedOption(); // we can not use directly because we want the text of it not the web element itself
        String actualYear = yearDropdown.getFirstSelectedOption().getText(); // we can now get the text
        String actualMonth = monthDropdown.getFirstSelectedOption().getText();
        String actualDay = dayDropdown.getFirstSelectedOption().getText();

        //create assertions
        Assert.assertTrue(actualYear.equals(expectedYear), "ACTUAL YEAR NOT EQUAL EXPECTED YEAR!"); // it is optional failure message to use but use only failing test
    ...
    public void dropdown_task7() throws InterruptedException {
        ...
        //5. Select California
        Thread.sleep(2000);
        stateDropdown.selectByIndex(5); // this will select the california
